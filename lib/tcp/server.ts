...
import { handleTcpConnection } from './connection.ts';

export type TcpServerEvents = EventEmitter & {
  on(
    event: 'message',
    listener: (data: { socket: Socket; message: Buffer; clientLabel: string }) => void
  ): EventEmitter;
...
export const createTcpServer = (port: number): TcpServer => {
